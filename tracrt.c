...
int pingInfinito(unsigned long saddr,unsigned long daddr ){
	int x=0;
	int atingido = 0;
	int ttl=20;
	printf("\n");
...
int main(int argc, char **argv) {
   ...
   do {
    ...
    while(opc){
    	executa = 1;
    	if(opc==1)
    		trace(system("ifconfig | grep 'inet end' | awk '{print $3}' | grep -v '127'"), inet_addr(ip), 0);
    	if(opc==2){
    		pingInfinito(system("ifconfig | grep 'inet end' | awk '{print $3}' | grep -v '127'"), inet_addr(ip));
    	}
    	if(opc==3)
    		pingReduzido(system("ifconfig | grep 'inet end' | awk '{print $3}' | grep -v '127'"), inet_addr(ip));
    	do {
    		printf("\nEscolha seu comando:\n 1-Traceroute\n2-Ping infinito\n3-Ping de 4 pacotes\n0-Sair\n");
    		scanf(" %d",&opc);
    	}
